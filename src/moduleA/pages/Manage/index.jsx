...
const Manage = () => {
  ...
  const update = () => {
    ...
    Fetch(
      ...
      .then((res) => {
        if (res.data.list) setmembers(res.data.list)
        /*  else {
            Taro.showToast({
                title:'没有更多了哦',
                icon:'none'
    ...
    Taro.showModal({
      ...
      success: function (res) {
        if (res.confirm) {
          ...
          console.log('用户点击确定',address, user_id)
          Fetch(
            '/role/change',
            {
    ...
    <>
      <View className="manage">
        <View className="column m-list" /* onScrollToLower={handleScroll} */>
          {members &&
            members.map((item) => {
              const { id, username, address } = item
              return (
                <View
                  className="member"
                  key="item"
                ...
                >
                  ...
                  <Text className="check" onClick={() => tohistory(id)}>
                    查看变更记录
                  </Text>
                </View>
              )
