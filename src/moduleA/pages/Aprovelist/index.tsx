...
const AList = () => {
  ...
  const dictionary = ['非在册队员', '申请队员', '岗前培训']
  useDidShow(() => {
    setforms([])
    fetchData(0, page)
  })
  ...
  function toaprove(item) {
    const { id } = item
    Taro.navigateTo({
      url: `/moduleA/pages/Aprove/index?id=${id}`,
    })
  }
  ...
  return (
    <>
      <View className="Form_i" onClick={chooseAddress}>
        <AtList>
          <AtListItem title="选择省市" extraText={address} />
        </AtList>
      </View>
      {showPicker && (
        <AddressPicker
          address={address}
          onAConfirm={chooseAddress}
          onPick={handlePick}
          withCity={false}
        ></AddressPicker>
      )}
